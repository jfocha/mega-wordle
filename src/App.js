...
const App = () => {
  ...
  const [remainingGuesses, setRemainingGuesses] = useState(7); // Number of remaining guesses
  const [showLandingPage, setShowLandingPage] = useState(true); // Landing Page flag
  const [animationTriggered, setAnimationTriggered] = useState(false); // Title animation flag
  const now = new Date();
  ...
  const startTitleAnimation = useCallback(() => {
    setFlippedLetters([]); // Reset the flipped letters
  ...
  }, [currentLevelIndex, targetWord.length, level, saveGame, startTitleAnimation]);
  ...
  }, [remainingGuesses, isGameOver, targetWord.length, saveGame]);
